...
public class WMG_Axis : WMG_GUI_Functions {
	...
	public void ChangeOrientation() {
		...
		otherAxis.ChangeOrientationEnd(tLabelType, tAxisMaxValue, tAxisMinValue, tAxisNumTicks, tnumDecimalsAxisLabels,
		                               ...
		                               tSetLabelsUsingMaxMin, tAxisLabelSpacing, tAxisTitleString, tLabels);
	}
	...
	public void ChangeOrientationEnd(labelTypes tLabelType, float tAxisMaxValue, float tAxisMinValue, int tAxisNumTicks, int tnumDecimalsAxisLabels,
	                                 ...
	                                 bool tSetLabelsUsingMaxMin, float tAxisLabelSpacing, string tAxisTitleString, List<string> tLabels) {
	...
	public void UpdateAxesGridsAndTicks() {
		...
		if (!hideTicks) {
			...
			if (isY) {
				foreach (WMG_Node node in graph.getYAxisTicks()) {
					changeSpriteSize(node.objectToScale, Mathf.RoundToInt(graph.tickSize.y), Mathf.RoundToInt(graph.tickSize.x));
				}
			}
			else {
				foreach (WMG_Node node in graph.getXAxisTicks()) {
					changeSpriteSize(node.objectToScale, Mathf.RoundToInt(graph.tickSize.x), Mathf.RoundToInt(graph.tickSize.y));
				}
			}
		}
		...
		float axisRepos = 0;
		if (!AxisArrowTopRight) AxisLinePaddingTot -= AxisLinePadding;
		else axisRepos += AxisLinePadding / 2f;
		if (!AxisArrowBotLeft) AxisLinePaddingTot -= AxisLinePadding;
		else axisRepos -= AxisLinePadding / 2f;
	...
	public void UpdateAxesMinMaxValues() {
		...
		if (MinAutoGrow || MaxAutoGrow || MinAutoShrink || MaxAutoShrink) {
			if (min == max || min == Mathf.Infinity || max == Mathf.NegativeInfinity) return;
	...
	public void UpdateAxesLabels() {
		...
		if (isY) LabelNodes = graph.getYAxisLabels();
		else LabelNodes = graph.getXAxisLabels();
		...
		for (int i = 0; i < axisLabels.Count; i++) {
			...
			// Set the labels
			if (SetLabelsUsingMaxMin) {
				float num = AxisMinValue + i * (AxisMaxValue - AxisMinValue) / (LabelNodes.Count-1);
				if (i == 0) num = AxisMinValue;
				
				if (graph.graphType == WMG_Axis_Graph.graphTypes.bar_stacked_percent && ((isY && graph.orientationType == WMG_Axis_Graph.orientationTypes.vertical) 
				                                                                         || (!isY && graph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal))) {
					num = i / (LabelNodes.Count-1f) * 100f;
				}
				float numberToMult = Mathf.Pow(10f, numDecimalsAxisLabels);
				
				axisLabels.SetValNoCb(i, (Mathf.Round(num*numberToMult)/numberToMult).ToString(), ref _axisLabels);
				if (graph.graphType == WMG_Axis_Graph.graphTypes.bar_stacked_percent && ((isY && graph.orientationType == WMG_Axis_Graph.orientationTypes.vertical) 
				                                                                         || (!isY && graph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal))) {
					axisLabels.SetValNoCb(i, (axisLabels[i] + "%"), ref _axisLabels);
				}
			}
			if (LabelType == labelTypes.groups) {
				axisLabels.SetValNoCb(i, graph.groups[i], ref _axisLabels);
			}
...
//			graph.ResumeCallbacks();
	...
	public void setLabelScales(float newScale) {
		foreach (WMG_Node node in (isY ? graph.getYAxisLabels() : graph.getXAxisLabels())) {
